...
source and committed** on branch `harden-save-and-api-3.1.4`. The only things
left are a real packaged build and the release/publish steps.
...
- Tests: `python -m unittest testing.test_saa_smoke` → **32/32 pass**.
...
- The `dist/` EXE is **still the old 3.1.3 build** — it must be rebuilt (below).
...
1. **Build the 3.1.4 package** (needs `uv`, ImageMagick `magick`, Inno Setup 6):
   ```powershell
   .\build.ps1
   ```
   Artifacts:
...
2. **Validate the collapsed terminal in the frozen build** — this is the one
   thing not yet proven end-to-end. Launch the built EXE, open the in-app CLI
   terminal, and confirm the REPL runs (the main EXE relaunches itself with
   `--cli` under WinPTY). If ConPTY misbehaves on a target Win11 box, the WinPTY
   backend is already forced in `ws_server.terminal_session_winpty`.
3. **Smoke-test** the installed app: window opens, books import, saves persist to
   `%LOCALAPPDATA%\Lone Wolf Action Assistant`, combat runs, no console tab flashes.
4. **Merge / release:** open a PR from `harden-save-and-api-3.1.4` into `main`,
   or fast-forward `main`; then package and publish per
...
```powershell
python -m unittest testing.test_saa_smoke      # 32 pass
